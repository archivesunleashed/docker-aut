...
```
docker run --rm -it aut /spark/bin/spark-shell --jars /aut/aut-0.91.0-fatjar.jar --conf spark.network.timeout=100000000 --conf spark.executor.heartbeatInterval=6000s
```
...
```
docker run --rm -it aut /spark/bin/pyspark --py-files /aut/aut-0.91.0.zip --jars /aut/aut-0.91.0-fatjar.jar
```
